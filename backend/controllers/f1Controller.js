...
// const F1_API_BASE_URL = "https://f1api.dev/api"; // Note: This may not work for practice sessions
// const F1_CONNECT_API_BASE_URL = "https://f1connectapi.vercel.app/api";

// const f1Controller = {
//   async insertInitialData(req, res) {
//     try {
//       const { season, round, session } = req.body;
//       if (!season) {
//         return res
//           .status(400)
...
//       try {
//         const practiceUrl = `${F1_CONNECT_API_BASE_URL}/${season}/${round}/${session}?limit=10`; // Use the working API
//         console.log(`Fetching from: ${practiceUrl}`);
...
//       const url = `${F1_CONNECT_API_BASE_URL}/${year}/${round}/${session}?limit=10`;
//       const { data } = await axios.get(url);

//       if (!data?.races?.fp1Results || data.races.fp1Results.length === 0) {
//         return res.status(404).json({
//           status: "error",
//           message: "No practice session data found from external API",
//         });
//       }

//       // Insert data into MongoDB if not already present
//       const practiceData = data.races.fp1Results;
//       for (const practice of practiceData) {
//         try {
//           const driverDoc = await Driver.findOneAndUpdate(
//             { driverId: practice.driverId },
...
//           const sessionDateTime = new Date(
//             `${data.races.fp1Date}T${data.races.fp1Time}`
//           );
...
//               circuitLength: data.races.circuit.circuitLength,
//               lapRecord: data.races.circuit.lapRecord,
//               firstParticipationYear: data.races.circuit.firstParticipationYear,
//               corners: data.races.circuit.corners,
//               fastestLapDriverId: data.races.circuit.fastestLapDriverId,
//               fastestLapTeamId: data.races.circuit.fastestLapTeamId,
//               fastestLapYear: data.races.circuit.fastestLapYear,
//               url: data.races.circuit.url,
//             },
//           };
//           await PracticeSession.findOneAndUpdate(
//             {
...
//     } catch (error) {
//       console.error(
//         "Error fetching practice session from external API:",
//         error.message
//       );
...
const f1Controller = {
  async insertInitialData(req, res) {
    try {
      const { season, round, session } = req.body; // Extract round and session from body
      if (!season || !round || !session) {
        return res.status(400).json({
          status: "error",
          message: "Season, round, and session are required",
        });
      }
      console.log(
        `Fetching F1 data for season ${season}, round ${round}, session ${session}...`
      );
      ...
      let practiceResponse = null;
      try {
        const practiceUrl = `${F1_CONNECT_API_BASE_URL}/${season}/${round}/${session}?limit=10`; // Use parameters from req.body
        console.log(`Fetching from: ${practiceUrl}`);
        practiceResponse = await fetchWithRetry(practiceUrl);
        console.log(
          "Practice Session API Response:",
...
exports.fetchF1Data = async (filterParam) => {
  const limit = 10;
